...
## 4.Куб cube.done (Выполнение мероприятий по месяцам)
...
stage.	В данном ETL используется tmp.for_basic_metrics, который уже объединяет данные из stage.event_task и stage.disp_info.
...
3.3 Порядок сборки
Пересоздать tmp.done_task_month_source
...
– Вычислить фильтры: region_smo_cd (первые 2 цифры СМО), profile_cd, dn_group_cd, smo_cd, mo_cd
– Ограничить диапазон лет 

Очистить cube.done (TRUNCATE)

Рассчитать cube.done
– В подзапросе prepared выполнить CROSS JOIN с двумя массивами:

metric_kind IN ('p', 'c') – разделение на прошлый (p) и текущий (c) год

mask от 0 до 31 (5 бит) – для генерации всех комбинаций замены фильтров на '0' (все значения)
...
Источник задач	INSERT INTO tmp.done_task_month_source	Если нужно изменить фильтрацию задач (например, добавить статусы, изменить период отбора) – здесь.
Вычисление closed_y, closed_q, closed_m	INSERT INTO tmp.done_task_month_source	Если необходимо смещение года или квартала.
Фильтры (регион, профиль, группа, СМО, МО)	INSERT INTO tmp.done_task_month_source	Если изменилась логика получения кода региона или обработки NULL/0.
Генерация metric_kind	CROSS JOIN (SELECT arrayJoin(['p','c']) AS metric_kind)	Если нужно добавить другие виды сравнения (например, ещё один предыдущий год).
Битовая маска (маска фильтров)	CROSS JOIN (SELECT arrayJoin(range(32)) AS mask)	Если добавились новые фильтры (увеличить число масок до 2^N).
Вычисление f_y для metric_kind='p'	if(metric_kind='p', closed_y+1, closed_y)	Если нужно изменить сдвиг года для прошлого периода.
Агрегатные выражения для каждого месяца	uniqExactIf(task_id, ...) в основном SELECT	Если нужно изменить статусы или добавить другие метрики.
Итоговое HAVING	HAVING sum(...) <> 0	Если нужно изменить условие отсеивания пустых строк.
3.5 Правила изменения логики
Любое изменение логики должно фиксироваться в двух местах:

Сам SQL-блок в ETL-скрипте.
Раздел «Где менять логику расчётов» данной документации.
...
-- Убедиться, что проценты (если бы были) – но в этом кубе процентов нет.
4. Правила изменения логики (общие)
Любое изменение логики должно фиксироваться в двух местах:
